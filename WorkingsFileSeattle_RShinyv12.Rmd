...
```{r}
package <- c('rgeos','shinydashboard','RColorBrewer','leaflet','shinythemes','shiny','spatstat','raster','tmaptools','rgdal','sf','sp','tmap','tidyverse')
...
```{r}
table(downtown_listings_sp$Airconditioning)
```
...
server <- function(input, output) {
  ...
  queried_location <- eventReactive(input$run,{
    ...
    output$buffer_map <- renderLeaflet({ req(input$entered_address)
        ...
        working_map <- tm_shape(seattle_downtown_sp) + tm_borders(lwd = 3) + tm_fill(col = "#F6EAD3", alpha = 0.5) + tm_shape(entered_location_buffer, is.master = TRUE) + tm_polygons(col = "#FFA150",alpha = 0.3) + tm_shape(entered_address_sp, is.master = FALSE) + tm_markers() + tm_view(set.zoom.limits = c(15,18)) + tm_basemap("Esri.WorldTopoMap") + tm_shape(downtown_listings_sp, popup.vars = c("Room Type" = "room_type", "Property Type" = "property_type")) + tm_dots(col = input$colored_by, palette = color_palette)
        tmap_leaflet(working_map)
      }
    )
...
tmap_mode("view")
tm_shape(chosen_location) +
  tm_dots() + tm_view(set.zoom.limits = c(15,18), set.view = c(lon = tryLon+1, lat = tryLat, zoom = 15))
``` 
...
```{r}
tm_shape(zoning_generalized) +
  tm_polygons()
```

```{r}
proj4string(zoning_generalized)
proj4string(downtown_listings_sp)
```

```{r}
zoning_generalized_32148 <- spTransform(zoning_generalized, CRS("+init=epsg:32148"))
```

```{r}
proj4string(zoning_generalized_32148)
proj4string(downtown_listings_sp)
```

```{r}
table(zoning_generalized$CLASS_DESC)
```

```{r}
str(zoning_generalized)
```


```{r}
temp <- over(downtown_listings_sp,zoning_generalized_32148)

table(temp$ZONELUT_DE)
```

```{r}
temp_over <- as.data.frame(table(temp$OBJECTID))
temp_over$
```

```{r}
zoning_generalized_32148@data <- inner_join(zoning_generalized_32148@data, temp_over,
                                            by = c("OBJECTID" = "Var1"))
```

```{r}
zoning_generalized_32148 <- subset(zoning_generalized_32148, CLASS_DESC == "Downtown")
```

```{r}
nrow(zoning_generalized_32148@data)
```
...
tm_shape(zoning_generalized_32148) +
  tm_fill(col = "Freq",
          popup.vars =  c("Class Desc" = "CLASS_DESC", "Zone" = "ZONELUT_DE"),
          palette = "Blues",
          n = 8,
          style = "jenks") +
...
```{r}
tm_shape(zoning_generalized_32148) +
  tm_fill(col = "ZONELUT_DE") +
  tm_borders(alpha = 0.5)
```

```{r}
sum(zoning_generalized_32148$Freq)
```

```{r}
temp_zone <- table(zoning_generalized_32148$ZONELUT_DE)
temp_zone[temp_zone > 0 ]
```


```{r}
temptable <- table(temp$ZONELUT_DE)
temptable[temptable > 0]
```



```{r}
table(zoning_generalized$ZONELUT)
```

```{r}
temp <- over(downtown_listings_sp,seattle_downtown_sp)
tempLOL <- table(temp$S_HOOD)
tempLOL[tempLOL >0]
```
